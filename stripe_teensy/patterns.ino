...
struct CRGB colorWipe(long f, int pixelIndex) {
  if (f < 0) return NULL_COLOR;
  if (pixelIndex % NUM_PIXELS < f % NUM_PIXELS)
    return color1;
  return color2;
}
...
struct CRGB rainbowCycle(long f, int pixelIndex) {
  if (f < 0) return NULL_COLOR;
  return Wheel((f + (pixelIndex % NUM_PIXELS)*5) % 384);
}
...
struct CRGB colorChase(long f, int pixelIndex) {
 if (f < 0) return NULL_COLOR;
 int j = 9;
 if (pixelIndex%NUM_PIXELS >= f%NUM_PIXELS && pixelIndex%NUM_PIXELS <= f%NUM_PIXELS+j)
   return color1;
 return color2;
}
...
struct CRGB colorWipeMeter(long f, int pixelIndex) {
 ...
 if (f == -1) {
   params[0] = random(0,NUM_PIXELS);
   return NULL_COLOR;
 }
 
 if (pixelIndex % NUM_PIXELS < params[0]) 
   return color1;
 return color2;
